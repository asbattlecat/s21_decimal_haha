...
void set_1_bit(unsigned int *src, unsigned int index) { *src |= (1u << index); }
...
int get_bit(unsigned int src, unsigned int index) { return (src >> index) & 1; }
...
int get_exp(s21_decimal src) { return (src.bits[3] & EXP) >> 16; }